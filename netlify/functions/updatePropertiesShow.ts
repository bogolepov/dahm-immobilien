...
import { extractSchemaFromJson, zUpdatePropertiesShowInput } from '@scripts/zod';
...
async function processHandler(event: HandlerEvent, context: HandlerContext): Promise<HandlerResponse> {
	...
	const input = extractSchemaFromJson(zUpdatePropertiesShowInput, event.body);
	if (!input) return response(400, 'Bad request');

	for (const { id, show } of input) {
		const { data, error } = await writeSupabase.from(SUPABASE_PROPERTIES_TABLE).update({ show }).eq('id', id).select().single();
